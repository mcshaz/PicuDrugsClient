import { IEntityFixedInfusionDrug } from './../../../../db';
import { maxWeight, minWeight } from './../../../../db/src/helpers/fieldConstants';
...
export function getFixedDilutionsForPt(drug: IEntityFixedInfusionDrug, ageMonths: number, weightKg: number): IPatientFixedInfusionDrug | undefined {
    ...
    if (d.length === 1) {
        ...
        newVar.dilution.concentrations = d[0].concentrations.map((c) => {
            ...
            lastStopMins = nc.durationMinutes;
            return nc;
        });
    } else if (d.length > 1) {
