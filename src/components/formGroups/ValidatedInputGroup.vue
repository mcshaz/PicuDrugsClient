<template>
  <validation-provider v-slot="{ errors, changed, failedRules }" :name="pErrorLabel" :rules="rules" slim>
    <b-form-group :label-for="pInputName" :label-cols-lg="labelColsLg" :label-cols-xl="labelColsXl"
        :state="errors[0] ? false : (changed ? true : null)" label-align-lg="right">
      <template #label><slot name="label">{{ label }}</slot><span class="label-colon">:</span></template>
      <template #invalid-feedback><slot name="invalid-feedback" :failedRules="failedRules" :errors="errors">{{ errors[0] }}</slot></template>
      ...
      <b-input-group>
        ...
        <input :type="type" :min="min" :max="max" :step="step" :placeholder="placeholder" :required="required" :disabled="disabled"
            v-model="pValue" :id="pName" :name="pName" :class="{'custom-range': type==='range', 'form-control': type!=='range','is-invalid':errors[0],'is-valid':!errors[0]&&changed}" :autocomplete="autocomplete">
