<template>
    <div>
        <b-form-group label-for="nhi" label-cols-lg="2" label-cols-xl="2" label="NHI:" :state="$v.$error" >
            <input class="form-control" type="text" id="nhi" name="nhi" v-model.trim="nhi" placeholder="NHI"/>
            <template slot="invalid-feedback" v-if="$v.$invalid">
                <template v-if="$v.nhi.nhiChars===false">
...
import { defaultSim, getNHIVals } from '@/services/validation/getNHIVals';
