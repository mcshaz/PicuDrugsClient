...
import { IDrugDB, IEntityInfusion, IEntityFixedInfusionDrug, IEntityDrugAmpuleConcentration } from '@/services/drugDb';
...
export default class StatusEpilepsySvg extends Vue {
    ...
    private async getInfusions() {
        if (this.wtKg) {
            const pheny = await Promise.all([/* this.cvlAmioDbData,*/ this.pivPhenyDbData ]) as any as IEntityFixedInfusionDrug[];
            // note 600 = age months - this is not actually relevant for our current amiodarone protocols
            const wtSelectedInfusion = getFixedDilutionsForPt(pheny[0], 600, this.wtKg)!;
            this.pivPheny = transformFixedInfusions(this.wtKg, wtSelectedInfusion);
