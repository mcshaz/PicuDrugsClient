...
export class DrugsDBLocal extends Dexie {
    ...
    // Declare implicit table properties.
    // (just to inform Typescript. Instanciated by Dexie in stores() method)
    public wards!: Dexie.Table<IWard, number>; // number = type of the primkey
 // number = type of the primkey
    ...
    public deletionDates!: Dexie.Table<IDeletionDate, number>;
    ...
    private async updateLocalDataFromServer(): Promise<IDeletionDate[]> {
        const dbDates: IdbMods = {
            bolusDeletedUntil : null,
            infusionsDeletedUntil : null,
            wardDeletedUntil : null,
        } as any;
        return Promise.all([
            this.infusionDrugs.orderBy('LastUpdated').last().then((i) => dbDates.infusionsUpdatedUntil = (i || {} as any).LastUpdated || null),
            this.wards.orderBy('LastUpdated').last().then((w) => dbDates.wardUpdatedUntil = (w || {} as any).LastUpdated || null),
            this.bolusDrugs.orderBy('LastUpdated').last().then((b) => dbDates.bolusUpdatedUntil = (b || {} as any).LastUpdated || null),
    ...
    private async seedLocalDataFromServer() {
        ...
        const updates = await this.updateProvider.getUpdates({
