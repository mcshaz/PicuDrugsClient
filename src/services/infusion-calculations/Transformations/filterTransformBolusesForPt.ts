...
export function filterTransformBolusesForPt(boluses: IEntityBolusDrug[], weightKg: number): IBolusDrugVM[] {
    ...
    for (const b of boluses) {
        ...
        if (d.length === 1) {
            const newVar = mapProperties({} as IBolusDrugVM, b, ['bolusDrugId', 'adultMax', 'conc_ml', 'units', 'drugName', 'min']);
            newVar.dosePerKg = new NumericRange(d[0].weightMin, d[0].weightMax);
            newVar.patientDose = NumericRange.op_Multiply(newVar.dosePerKg, weightKg);
            newVar.patientDose.cap(newVar.adultMax);
            newVar.patientDose.floor(newVar.min);
            if (newVar.conc_ml) {
                newVar.patientVolume = NumericRange.op_Division(newVar.patientDose, newVar.conc_ml);
            }
