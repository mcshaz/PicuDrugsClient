...
export function getETTSize(ageInMonths: number, weight = 0.0, gestAgeBirth = 40.0): EttDetails {
  ...
  if (ett.InternalDiameter <= 2.5) {
    ett.SuctionWith = 6;
  } else if (ett.InternalDiameter === 3.0) {
    ett.SuctionWith = 7;
  } else if (ett.InternalDiameter <= 4.5) {
    ett.SuctionWith = 8;
  } else if (ett.InternalDiameter <= 6.0) {
    ett.SuctionWith = 10;
  } else {
    ett.SuctionWith = 12;
  }
