...
export class PdfTableValues {
    ...
    public createPdfStream(txt: string[], {fontSize = 12, getWidth = (_txt: string) => 0, rowNo = 0, fontPdfKey = "F1"} : ICoordData & IBTData & IWidthData) {
        ...
        return returnVar.map(rv => txt + ' ET\n');
    }
    public static simpleBT(txt: string, coord: [number, number], {fontSize = 12, fontPdfKey = "F1", getWidth = (_txt: string) => 0} : IBTData & ICoordData = {}) {
